...
DL_DIR = os.path.join(HERE, "deep_learning")
sys.path.insert(0, DL_DIR)
try:
    import torch
    from dl_models import load_best_rnn, load_best_cnn, BEST_RNN_NAME, BEST_CNN_NAME
    DL_AVAILABLE = True
except Exception:
    DL_AVAILABLE = False
...
with st.sidebar:
    ...
    menu_options = ["Khám phá dữ liệu", "So sánh mô hình", "Feature Selection", "Dự đoán trực tiếp"]
    menu_icons = ["bar-chart-line", "cpu", "funnel", "heart-pulse"]
    if DL_AVAILABLE:
        menu_options.append("Deep Learning")
        menu_icons.append("diagram-3")
...
HERO_TEXT = {
    ...
    "So sánh mô hình": ("🤖 So sánh mô hình phân loại",
                         "Huấn luyện & đối chiếu 4 mô hình: Logistic Regression, Random Forest, SVM, KNN."),
    "Feature Selection": ("🔍 Feature Selection theo Correlation",
    ...
    "Deep Learning (Cuối kỳ)": ("🧠 RNN/LSTM/GRU & CNN1D",
                                 "So sánh mô hình học sâu với baseline truyền thống, và dự đoán trực tiếp bằng mô hình LSTM/CNN1D tốt nhất."),
}
...
if selected == "Khám phá dữ liệu":
    ...
    with st.expander("❓ Không biết về Accuracy, Precision, Recall...? Bấm vào đây"):
        st.write(
    ...
    if submitted:
        ...
        with r1:
            if pred == 1:
                st.error(f" Dự đoán: **CÓ nguy cơ bệnh tim** (xác suất {proba[1]*100:.1f}%)")
            else:
    ...
    if not DL_AVAILABLE:
        st.error("Không tìm thấy thư viện `torch` hoặc thư mục `deep_learning/`. Cài đặt bằng `pip install torch` rồi chạy lại.")
    else:
        @st.cache_data
        def get_dl_comparison():
            path = os.path.join(DL_DIR, "results", "comparison_table.csv")
            return pd.read_csv(path)

        @st.cache_data
        def get_dl_summary():
            path = os.path.join(DL_DIR, "results", "summary.json")
            with open(path, "r", encoding="utf-8") as f:
                return json.load(f)

        @st.cache_resource
        def get_dl_scaler():
            with open(os.path.join(DL_DIR, "models", "scaler.pkl"), "rb") as f:
                return pickle.load(f)

        @st.cache_resource
        def get_dl_models():
            model_dir = os.path.join(DL_DIR, "models")
            return load_best_rnn(model_dir), load_best_cnn(model_dir)

        comp_df = get_dl_comparison()
        summary = get_dl_summary()

        st.markdown("Mở rộng bài toán bằng cách thử nghiệm thêm các mô hình học sâu "
                     "(RNN/LSTM/GRU và CNN1D, huấn luyện bằng PyTorch) và đối chiếu với các mô hình baseline "
                     "truyền thống (Logistic Regression, Random Forest, SVM, KNN, Decision Tree, MLP) trên cùng "
                     "bộ dữ liệu Cleveland — chia theo tỉ lệ train/val/test 60/20/20.")

        k1, k2, k3, k4 = st.columns(4)
        kpi(k1, summary["best_baseline"], "Baseline tốt nhất")
        kpi(k2, summary["best_rnn"], "RNN tốt nhất")
        kpi(k3, summary["best_cnn"], "CNN1D tốt nhất")
        kpi(k4, f"{summary['best_overall_f1']*100:.1f}%", f"Test F1 — {summary['best_overall']}")

        st.subheader("📊 So sánh toàn bộ mô hình (Baseline vs RNN/LSTM/GRU vs CNN1D)")
        fig = px.bar(
            comp_df.sort_values("Test F1", ascending=True),
            x="Test F1", y="Mô hình", color="Nhóm", orientation="h",
            color_discrete_map={
                "Baseline truyền thống": "#7b2ff7",
                "RNN/LSTM/GRU": "#d6249f",
                "CNN1D": "#f77737",
            },
        )
        ...
        insight(f"""
        <b>{summary['best_overall']}</b> ({summary['best_overall_group']}) vẫn cho Test F1 cao nhất
        (<b>{summary['best_overall_f1']*100:.2f}%</b>) trong toàn bộ 14 cấu hình mô hình đã thử. Dữ liệu Heart
        Disease là dữ liệu dạng bảng (tabular), không có cấu trúc chuỗi thời gian hay không gian thật sự —
        vì vậy RNN/CNN không phát huy được lợi thế thiết kế của chúng bằng các mô hình baseline truyền thống,
        dù vẫn đạt kết quả khá tốt (CNN1D tốt nhất đạt {comp_df.loc[comp_df['Mô hình']==summary['best_cnn'], 'Test F1'].values[0]*100:.1f}% F1).
        """)

        c1, c2 = st.columns(2)
        with c1:
            st.image(os.path.join(DL_DIR, "figures", f"rnn_{summary['best_rnn']}_curves.png"),
                      caption=f"Learning curves — {summary['best_rnn']}", use_container_width=True)
        with c2:
            st.image(os.path.join(DL_DIR, "figures", f"cnn_{summary['best_cnn']}_curves.png"),
                      caption=f"Learning curves — {summary['best_cnn']}", use_container_width=True)

        st.markdown("---")
        st.subheader("🩺 Dự đoán trực tiếp bằng LSTM & CNN1D tốt nhất")
        st.caption("Dùng lại thông số bệnh nhân giống trang 'Dự đoán trực tiếp' (encoding UCI gốc), "
                   "hệ thống sẽ tự quy đổi sang encoding mà 2 mô hình học sâu này được huấn luyện.")

        with st.form("predict_form_dl"):
            d1, d2, d3 = st.columns(3)
            age = d1.number_input("Tuổi", 20, 90, 50, key="dl_age")
            sex = d1.selectbox("Giới tính", [1, 0], format_func=lambda v: "Nam" if v == 1 else "Nữ", key="dl_sex")
            cp = d1.selectbox("Loại đau ngực (cp, 1-4)", [1, 2, 3, 4], key="dl_cp")
            trestbps = d2.number_input("Huyết áp nghỉ (trestbps)", 80, 220, 130, key="dl_trestbps")
            chol = d2.number_input("Cholesterol (chol)", 100, 600, 240, key="dl_chol")
            fbs = d2.selectbox("Đường huyết đói > 120 (fbs)", [0, 1], key="dl_fbs")
            restecg = d3.selectbox("Điện tâm đồ nghỉ (restecg)", [0, 1, 2], key="dl_restecg")
            thalach = d3.number_input("Nhịp tim tối đa (thalach)", 60, 220, 150, key="dl_thalach")
            exang = d3.selectbox("Đau ngực do gắng sức (exang)", [0, 1], key="dl_exang")
            oldpeak = st.slider("ST depression (oldpeak)", 0.0, 6.5, 1.0, 0.1, key="dl_oldpeak")
            slope = st.selectbox("Độ dốc ST (slope, 1-3)", [1, 2, 3], key="dl_slope")
            ca = st.selectbox("Số mạch máu hẹp (ca)", [0, 1, 2, 3], key="dl_ca")
            thal = st.selectbox("Thalassemia (thal)", [3, 6, 7], key="dl_thal")
            submitted_dl = st.form_submit_button("🔮 Dự đoán bằng Deep Learning", type="primary", use_container_width=True)

        if submitted_dl:
            thal_map = {3: 1, 6: 2, 7: 3}
            row = np.array([[
                age, sex, cp - 1, trestbps, chol, fbs, restecg,
                thalach, exang, oldpeak, slope - 1, ca, thal_map[thal],
            ]], dtype=np.float32)

            scaler = get_dl_scaler()
            row_scaled = scaler.transform(row)
            rnn_model, cnn_model = get_dl_models()

            x_rnn = torch.tensor(row_scaled.reshape(1, row_scaled.shape[1], 1), dtype=torch.float32)
            x_cnn = torch.tensor(row_scaled.reshape(1, 1, row_scaled.shape[1]), dtype=torch.float32)
            with torch.no_grad():
                proba_rnn = torch.sigmoid(rnn_model(x_rnn)).item()
                proba_cnn = torch.sigmoid(cnn_model(x_cnn)).item()

            r1, r2 = st.columns(2)
            with r1:
                st.markdown(f"**{summary['best_rnn']} (LSTM)**")
                if proba_rnn >= 0.5:
                    st.error(f" CÓ nguy cơ bệnh tim (xác suất {proba_rnn*100:.1f}%)")
                else:
                    st.success(f" KHÔNG có nguy cơ bệnh tim (xác suất {(1-proba_rnn)*100:.1f}%)")
            with r2:
                st.markdown(f"**{summary['best_cnn']} (CNN1D)**")
                if proba_cnn >= 0.5:
                    st.error(f" CÓ nguy cơ bệnh tim (xác suất {proba_cnn*100:.1f}%)")
                else:
                    st.success(f" KHÔNG có nguy cơ bệnh tim (xác suất {(1-proba_cnn)*100:.1f}%)")

            warn("""
            Đây là mô hình demo phục vụ mục đích học tập, <b>không phải công cụ chẩn đoán y khoa</b>.
            Việc quy đổi encoding giữa 2 bộ dữ liệu (bài giữa kỳ dùng encoding UCI gốc, bài cuối kỳ dùng
            encoding đã số-hoá lại từ 0) là suy đoán hợp lý dựa trên tài liệu UCI, không phải ánh xạ chính thức.
            """)
